...
module.exports.run = async function ({ api, event, args }) {
  ...
  if (type === "message_reply" && messageReply.attachments[0]?.type === "photo") {
    ...
    if (!question) {
      api.sendMessage("❌ فشل تحويل الصورة إلى نص. يرجى المحاولة مرة أخرى مع صورة أوضح.", threadID, messageID);
      return;
    }
  } else {
  ...
  try {
    ...
    for (let i = 0; i < pinterestImageUrls.length; i++) {
      ...
      try {
        ...
        console.error("خطأ في جلب صورة بينتيريست:", error);
      }
    }

    const formattedBardAnswer = `📝 ميري ${formatFont(bardMessage)}`;
